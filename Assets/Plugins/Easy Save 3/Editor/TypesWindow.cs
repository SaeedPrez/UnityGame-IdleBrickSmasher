    ...
    public class TypesWindow : SubWindow
    {
        ...
        private void Init()
        {
            ...
            var assemblies = AppDomain.CurrentDomain.GetAssemblies()
                .Where(assembly => !assembly.FullName.Contains("Editor") && assembly.FullName != "ES3" && !assembly.FullName.Contains("ES3"))
                .OrderBy(assembly => assembly.GetName().Name).ToArray();
        ...
        private string GenerateWrites()
        {
            ...
            for (var i = 0; i < fields.Length; i++)
            {
                ...
                var es3TypeParam = HasExplicitES3Type(es3Type) && writeByRef == "" && !field.MemberType.IsEnum ? ", " + es3Type.GetType().Name + ".Instance" :
                    writeByRef == "" ? ", ES3Internal.ES3TypeMgr.GetOrCreateES3Type(typeof(" + GetFullTypeName(field.MemberType) + "))" : "";
        ...
        private string GenerateReads()
        {
            ...
            for (var i = 0; i < fields.Length; i++)
            {
                ...
                var es3TypeParam = HasExplicitES3Type(field.MemberType) ? ES3TypeMgr.GetES3Type(field.MemberType).GetType().Name + ".Instance" : "";
                // If this is static, access the field through the class name rather than through an instance.
                var instance = field.IsStatic ? GetFullTypeName(type) : "instance";
