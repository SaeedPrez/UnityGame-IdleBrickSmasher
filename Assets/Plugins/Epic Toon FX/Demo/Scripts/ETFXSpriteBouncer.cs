...
namespace EpicToonFX
{
    public class ETFXSpriteBouncer : MonoBehaviour
    {
        ...
        private float scaleTimer; // Timer for scaling animation
        ...
        private void Start()
        {
            ...
            if (startScale.y != 1f)
            {
                var adjustedScale = startScale.y / scaleAmount;
        ...
        private void Update()
        {
            ...
            var t = Mathf.Clamp01(scaleTimer / scaleDuration); // Calculate the interpolation factor with a clamp

            var verticalScale = Mathf.Lerp(startScale.y, startScale.y * scaleAmount, t) + Mathf.PingPong(scaleTimer / scaleDuration, 0.1f);
            ; // Interpolate the vertical scale
            var newScale = new Vector3(startScale.x, verticalScale, startScale.z); // Calculate the new scale
